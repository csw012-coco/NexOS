...
static uint8_t g_sched_root_session_turn = 1u;
...
uint32_t sched_policy_update_wake_times(uint32_t current_ticks) {
    ...
    /* Check the root user session outside the spawned job runtime slots. */
 ...
 * Returns a job runtime slot, or -2 if none is eligible.
 */
static int32_t sched_policy_select_job_runtime(void) {
    /* Round-robin through spawned job runtimes. */
...
int32_t sched_policy_select_next(void) {
    int root_session_ready = g_user_session.process.state == PROCESS_STATE_READY &&
                             sched_policy_pid_allowed(&g_user_session.process);
    int32_t job_slot;

    if (g_sched_mode == SCHED_MODE_INTERACTIVE && root_session_ready && g_sched_root_session_turn != 0u) {
        g_sched_root_session_turn = 0u;
        return -1;  /* Caller knows -1 = root user session. */
    }

    job_slot = sched_policy_select_job_runtime();
    if (job_slot >= 0) {
        g_sched_root_session_turn = 1u;
        return job_slot;
    }

    if (root_session_ready) {
        g_sched_root_session_turn = 0u;
        return -1;
    }
...
int sched_policy_get_stats(struct sched_policy_stats *out) {
    ...
    out->total_processes = 1 + USER_PROCESS_LIMIT;  /* root session + job runtime slots */
    out->ready_processes = ready;
    out->sleeping_processes = sleeping;
