...
void *mmap(void *addr,
    ...
    result = syscall4(SYS_MMAP, (uint64_t)(uintptr_t)&request, 0, 0, 0);
...
int shm_unlink(const char *name) {
    return syscall4(SYS_SHM_UNLINK, (uint64_t)(uintptr_t)name, 0, 0, 0) ? 0 : -1;
}

mqd_t mq_open(const char *name, int flags) {
    return (mqd_t)syscall4(SYS_MQ_OPEN,
                           (uint64_t)(uintptr_t)name,
                           (uint32_t)flags,
                           0,
                           0);
...
int mq_unlink(const char *name) {
    return syscall4(SYS_MQ_UNLINK, (uint64_t)(uintptr_t)name, 0, 0, 0) ? 0 : -1;
}
...
int mq_send(mqd_t queue, const void *data, size_t size, int flags) {
    ...
    for (;;) {
        result = (int64_t)syscall4(SYS_MQ_SEND,
                                   (uint32_t)queue,
...
int mq_receive(mqd_t queue, void *data, size_t capacity, int flags) {
    ...
    buffer.flags = (uint32_t)flags;
    for (;;) {
        result = (int64_t)syscall4(SYS_MQ_RECEIVE,
                                   (uint32_t)queue,
                                   (uint64_t)(uintptr_t)&buffer,
                                   0,
                                   0);
...
sem_t sem_open(const char *name, unsigned int initial_value, int flags) {
    return (sem_t)syscall4(SYS_SEM_OPEN,
                           (uint64_t)(uintptr_t)name,
                           initial_value,
                           (uint32_t)flags,
                           0);
...
int sem_unlink(const char *name) {
    return syscall4(SYS_SEM_UNLINK, (uint64_t)(uintptr_t)name, 0, 0, 0) ? 0 : -1;
}

int sem_trywait(sem_t sem) {
    return (int64_t)syscall4(SYS_SEM_TRYWAIT, (uint32_t)sem, 0, 0, 0) > 0 ? 0 : -1;
}
...
int sem_wait(sem_t sem) {
    ...
    for (;;) {
        result = (int64_t)syscall4(SYS_SEM_TRYWAIT, (uint32_t)sem, 0, 0, 0);
        if (result < 0) {
            return -1;
        }
        if (result > 0) {
...
int sem_post(sem_t sem) {
    return syscall4(SYS_SEM_POST, (uint32_t)sem, 0, 0, 0) ? 0 : -1;
}
...
long lseek(int fd, long offset, int whence) {
    return (long)syscall4(SYS_SEEK, (uint64_t)fd, (uint64_t)offset, (uint64_t)whence, 0);
}
...
pid_t fork(void) {
    return (pid_t)syscall4(SYS_FORK, 0, 0, 0, 0);
}
...
int mkfifo(const char *path) {
    return (int)syscall4(SYS_MKFIFO, (uint64_t)(uintptr_t)path, 0, 0, 0);
}
...
int gfx_blit(const uint32_t *pixels,
    ...
    blit.flags = 0u;
    return (int)syscall4(SYS_GFX, SYS_GFX_BLIT, (uint64_t)(uintptr_t)&blit, 0, 0);
}
...
int gui_input_grab(void) {
    return (int)syscall4(SYS_GUI_EVENT, SYS_GUI_EVENT_GRAB, 0, 0, 0);
}

int gui_input_release(void) {
    return (int)syscall4(SYS_GUI_EVENT, SYS_GUI_EVENT_RELEASE, 0, 0, 0);
}
