...
static int env_store_pair_local(uint32_t slot,
                                const char *name,
                                const char *value) {
    uint32_t pos = 0;
    uint32_t i = 0;
...
void __libc_env_init(char **envp) {
    g_env_count = 0;
    g_env_slots[0] = 0;
    while (envp != 0 && envp[g_env_count] != 0 &&
           g_env_count < NLIBC_ENV_MAX) {
...
int setenv(const char *name, const char *value, int overwrite) {
    ...
    if (env_store_pair_local((uint32_t)slot,
                             name,
                             value != 0 ? value : "") != 0) {
        if ((uint32_t)slot + 1u == g_env_count &&
            slot >= 0 && g_env_slots[slot] == 0) {
            g_env_count--;
        }
        return -1;
    }
...
int unsetenv(const char *name) {
    ...
    for (i = (uint32_t)slot; i + 1u < g_env_count; i++) {
        strlcpy(g_env_storage[i],
                sizeof(g_env_storage[i]),
                g_env_storage[i + 1u]);
