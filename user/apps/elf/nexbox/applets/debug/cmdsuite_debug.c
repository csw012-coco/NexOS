...
static int cmd_query_machine_info_local(struct syscall_machine_info *info) {
    if (info == NULL) {
        return 0;
    }
...
int cmd_doctor(int argc, char **argv) {
    ...
    if (cmd_query_machine_info_local(&machine)) {
        brand = skip_spaces_local(machine.cpu_brand);
        if (table) {
...
int cmd_minfo(void) {
    ...
    if (!cmd_query_machine_info_local(&info)) {
        write_err_str("minfo: machine info query failed\n");
        return 1;
    }
    brand = skip_spaces_local(info.cpu_brand);
