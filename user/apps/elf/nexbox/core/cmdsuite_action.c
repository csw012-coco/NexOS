...
static const struct nex_action_entry g_nex_actions[] = {
    ...
    {"editor.ed", "editor", "ed", "path?:path", "none", NEX_ACTION_CAP_FS_READ | NEX_ACTION_CAP_FS_WRITE, "fs.read fs.write", "open the line editor"},
    {"editor.vi", "editor", "vi", "path?:path", "none", NEX_ACTION_CAP_FS_READ | NEX_ACTION_CAP_FS_WRITE, "fs.read fs.write", "open the visual editor"},
    ...
    {"fs.dd", "storage", "dd", "if:path of:path opts?:word", "none", NEX_ACTION_CAP_FS_READ | NEX_ACTION_CAP_FS_WRITE | NEX_ACTION_CAP_BLOCK_INSPECT, "fs.read fs.write block.inspect", "copy bytes between files or block devices"},
    {"fs.df", "storage", "df", "flags?:word", "table/filesystems|text", NEX_ACTION_CAP_FS_INSPECT | NEX_ACTION_CAP_BLOCK_INSPECT, "fs.inspect block.inspect", "show filesystem space usage"},
    {"fs.du", "storage", "du", "flags?:word path?:path", "table/usage|text", NEX_ACTION_CAP_FS_READ | NEX_ACTION_CAP_FS_INSPECT, "fs.read fs.inspect", "show file or directory space usage"},
    ...
    {"fs.mkfs", "storage", "mkfs", "kind:word target:path", "none", NEX_ACTION_CAP_FS_WRITE | NEX_ACTION_CAP_BLOCK_INSPECT, "fs.write block.inspect", "format a filesystem"},
    ...
    {"gfx.fb", "gfx", "fb", "none", "record/framebuffer", NEX_ACTION_CAP_SYSTEM_INSPECT | NEX_ACTION_CAP_DEVICE_READ, "system.inspect device.read", "show framebuffer state"},
...
static const struct nex_mapper_entry g_nex_mapper_entries[] = {
    ...
    {"df", "fs.df", "flags?:word", "table/filesystems|text"},
    {"dd", "fs.dd", "if:path of:path opts?:word", "none"},
    {"dmesg", "debug.kmsg", "none", "text/kmsg"},
    {"dns", "net.dns", "host:host type?:word", "dns-answer"},
    {"doctor", "system.doctor", "flags?:word", "record/diagnostics|table/diagnostics"},
    {"du", "fs.du", "flags?:word path?:path", "table/usage|text"},
    {"ed", "editor.ed", "path?:path", "none"},
    {"fb", "gfx.fb", "none", "record/framebuffer"},
    {"file", "file.type", "flags?:word path:path", "record/file-type|table/file-type"},
    ...
    {"mkfs", "fs.mkfs", "kind:word target:path", "none"},
    {"mkfs.nxfs", "fs.mkfs", "target:path", "none"},
    {"mkfs.fat", "fs.mkfs", "flags?:word target:path", "none"},
    ...
    {"vi", "editor.vi", "path?:path", "none"},
    {"vim", "editor.vi", "path?:path", "none"},
...
static int nex_action_path_starts_with(const char *path, const char *prefix) {
    uint32_t i = 0;
...
static const char *nex_action_first_path_arg(int argc, char **argv) {
    int i;
...
const struct nex_action_entry *nex_action_find_for_invocation(const char *command, int argc, char **argv) {
    if (streq_ignore_case_local(command, "cat")) {
        const char *path = nex_action_first_path_arg(argc, argv);

        if (nex_action_path_starts_with(path, "/proc/")) {
            return nex_action_find("proc.read_node");
        }
        if (nex_action_path_starts_with(path, "/event/")) {
            return nex_action_find("event.read");
        }
        if (nex_action_path_starts_with(path, "/dev/")) {
            return nex_action_find("device.read");
        }
        if (path != NULL) {
...
static int nex_action_run(const struct nex_action_entry *action, int argc, char **argv) {
    ...
    return cmdsuite_run_backing_command(action->command, run_argc, run_argv);
}
