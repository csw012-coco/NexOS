...
int ush_execute_command_core(char *cwd, const char *line, int background) {
    ...
    if (background && (streq_local(line, "exit") ||
                       ...
                       streq_local(line, "clear") ||
                       streq_local(line, "su") ||
                       starts_with_local(line, "su ") ||
                       streq_local(line, "login") ||
                       starts_with_local(line, "login ") ||
    ...
    if (streq_local(line, "clear")) {
        clear();
        return 0;
    }
    /* Identity-changing commands must replace this shell.  Running them as
     * children leaves the old shell waiting and consumes a process slot for
     * the entire new interactive session. */
    if (streq_local(line, "su") || starts_with_local(line, "su ") ||
        streq_local(line, "login") || starts_with_local(line, "login ")) {
        char command[USH_LINE_MAX + 16u];
        int rc;

        if (snprintf(command, sizeof(command), "/cmd/nexbox %s", line) < 0) {
            return 1;
        }
        rc = exec_replace(command);

        if (rc != 0) {
            ush_write_colored_err(ush_dispatch_ansi_error, "identity: exec failed rc=");
            eprintf("%d\n", rc);
            return 1;
        }
        return 0;
...
int ush_execute_with_redirection(char *cwd, const struct ush_command_spec *spec, int background) {
    ...
    if (spec->input[0] == '\0' &&
        ...
        return ush_execute_command_core(cwd, spec->command, background);
    }
