...
static int ush_try_shebang_command(const char *line, const char *path) {
    char command[256];
    int rc;
    ...
    rc = spawn(command, SYS_SPAWN_ELF, 0);
    if (rc == 0) {
        return 1;
    }
...
static int ush_try_wav_command(const char *line, const char *path) {
    char command[256];
    int rc;
...
static int ush_try_external_command(char *cwd, const char *line, int *handled_out) {
    char token[64];
    char command[256];
    ...
    if (contains_char_local(token, '.') &&
        ...
        rc = spawn(action_command, SYS_SPAWN_ELF, 0);
        if (rc == 0) {
    ...
    if (explicit_path) {
        ...
        if (ush_try_shebang_command(line, token)) {
            if (handled_out != NULL) {
                *handled_out = 1;
            }
            return 0;
        }
        if (ush_try_wav_command(line, token)) {
            if (handled_out != NULL) {
                *handled_out = 1;
            }
            return 0;
...
static int ush_save_one_stdio(uint32_t saved[3], uint32_t fd_index, const char *path) {
    if (saved == NULL || fd_index > 2u || path == NULL || saved[fd_index] != 0xffffffffu) {
        return 0;
    }
    saved[fd_index] = (uint32_t)open(path, 0);
    return (int)saved[fd_index] >= 0;
}

static int ush_save_stdio(uint32_t saved[3], int save_stdin, int save_stdout, int save_stderr) {
    ush_init_saved_stdio(saved);
    if (save_stdin && !ush_save_one_stdio(saved, 0u, "/dev/stdin")) {
        return 0;
    }
    if (save_stdout && !ush_save_one_stdio(saved, 1u, "/dev/stdout")) {
        ush_restore_stdio(saved);
        return 0;
    }
    if (save_stderr && !ush_save_one_stdio(saved, 2u, "/dev/stderr")) {
        ush_restore_stdio(saved);
        return 0;
    }
    return 1;
...
static int ush_execute_command_core(char *cwd, const char *line) {
    ...
    }
    {
        ...
        int function_rc = ush_try_function_call_local(cwd, line, 0, &function_handled);
    ...
    }
    {
        ...
        handled = 0;
        status = ush_try_external_command(cwd, line, &handled);
        if (handled) {
...
static int ush_execute_with_redirection(char *cwd, const struct ush_command_spec *spec) {
    uint32_t saved[3];
    int rc;
    ...
    rc = ush_execute_command_core(cwd, spec->command);
    ush_restore_stdio(saved);
    return rc;
}
...
static int ush_execute_pipeline(char *cwd,
    ...
    for (i = 0; i < stage_count; i++) {
        ...
        rc = ush_execute_command_core(cwd, stages[i].command);
        if (next_write >= 0) {
...
static int ush_execute_line_core(char *cwd, const char *line) {
    ...
    if (!ush_expand_variables_local(line, expanded_line, sizeof(expanded_line))) {
    ...
    if (pipeline_stage_count == 1u) {
        return ush_execute_with_redirection(cwd, &pipeline_stages[0]);
    }
