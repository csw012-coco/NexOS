...
void i386_scheduler_task_init(struct scheduler_task *task,
    ...
    task->fpu_valid = 0u;
    task->address_space.kernel_root = i386_paging_kernel_root();
    task->address_space.user_root = root;
    ...
    process_set_capabilities(&task->process, PROCESS_CAP_SYS_ADMIN);
...
int i386_scheduler_run_loaded(const struct process_loaded_image *image,
    ...
    if (!i386_scheduler_run_one((uint32_t)image->entry,
                                ...
                                (uint32_t)image->root,
                                image->name) ||
        !process_scheduler_snapshot(0u, snapshot)) {
