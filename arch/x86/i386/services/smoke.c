...
int smoke_services_run_test32_strict_mm(void) {
    ...
    if (config.ops == 0 || config.ops->run_command == 0) {
        boot_services_log("test32: strict MM smoke unavailable");
        return 0;
    }
    ...
    boot_services_log("test32: strict MM PASS");
    return 1;
}
...
int smoke_services_run_nexbox32_full(void) {
    static const char *const commands[] = {
        ...
        "/cmd/nexbox sudo /cmd/nexbox echo sudo-ok",
    ...
    boot_user_services_init_config(&config);
    if (config.ops == 0 || config.ops->run_command == 0) {
        boot_services_log("nexbox32: applet smoke unavailable");
        return 0;
    }
    ...
    boot_services_log("test32: fork smoke PASS");
    ...
    boot_services_log("test32: fork shared mmap PASS");
    ...
    boot_services_log("test32: fork wait exec PASS");
    ...
    boot_services_log("nexbox32: full applet smoke PASS");
    return 1;
}

static int smoke_services_bytes_equal(const uint8_t *lhs, const uint8_t *rhs, uint32_t size) {
    if (lhs == 0 || rhs == 0) {
        return 0;
    }
...
static int smoke_services_name_starts_with(const char *name, const char *prefix) {
    if (name == 0 || prefix == 0) {
        return 0;
    }
    while (*prefix != '\0') {
...
static struct block_device *smoke_services_find_block_device_with_prefix(const char *prefix) {
    for (uint32_t i = 0u; i < blockdev_count(); i++) {
        struct blockdev_info info;
        struct block_device *dev = blockdev_get(i);

        if (dev == 0 || blockdev_get_info(i, &info) != 0 || info.name[0] == '\0') {
            continue;
        }
        if (smoke_services_name_starts_with(info.name, prefix)) {
            return dev;
        }
    }
    return 0;
...
static int smoke_services_run_block_rw(const char *prefix,
                                   const char *read_log,
    ...
    struct block_device *dev = smoke_services_find_block_device_with_prefix(prefix);
    uint64_t lba;

    if (dev == 0 || dev->block_size != 512u || dev->block_count < 2u ||
        dev->write == 0 || dev->read == 0) {
        boot_services_log("block: rw smoke unavailable");
        return 0;
    }
    lba = dev->block_count - 1u;
    if (blockdev_read(dev, lba, 1u, original) != 0) {
        boot_services_log("block: rw smoke read failed");
        return 0;
    }
    boot_services_log(read_log);
    for (uint32_t i = 0u; i < sizeof(pattern); i++) {
        pattern[i] = (uint8_t)(0xa5u ^ (uint8_t)i);
    }
    if (blockdev_write(dev, lba, 1u, pattern) != 0) {
        boot_services_log("block: rw smoke write failed");
        return 0;
    }
    boot_services_log(write_log);
    if (blockdev_flush(dev) != 0) {
        (void)blockdev_write(dev, lba, 1u, original);
        (void)blockdev_flush(dev);
        boot_services_log("block: rw smoke flush failed");
        return 0;
    }
    boot_services_log(flush_log);
    if (blockdev_read(dev, lba, 1u, verify) != 0 ||
        !smoke_services_bytes_equal(pattern, verify, sizeof(pattern))) {
        (void)blockdev_write(dev, lba, 1u, original);
        (void)blockdev_flush(dev);
        boot_services_log("block: rw smoke write/readback failed");
        return 0;
    }
    if (blockdev_write(dev, lba, 1u, original) != 0 ||
        blockdev_flush(dev) != 0 ||
        blockdev_read(dev, lba, 1u, verify) != 0 ||
        !smoke_services_bytes_equal(original, verify, sizeof(original))) {
        boot_services_log("block: rw smoke restore failed");
        return 0;
    }
    boot_services_log(restore_log);
    boot_services_log(ok_log);
    return 1;
}

static int smoke_services_run_ahci_rw(void) {
    return smoke_services_run_block_rw("ahci",
                                   "ahci: read smoke OK",
                                   "ahci: write smoke OK",
                                   "ahci: flush smoke OK",
...
static int smoke_services_run_usb_rw(void) {
    if (smoke_services_run_block_rw("usbmsc",
    ...
    return smoke_services_run_block_rw("xusbmsc",
...
static int smoke_services_run_usb_hid(void) {
    uint32_t ehci_count;
    uint32_t xhci_count;
...
static int smoke_services_run_rtl8139(void) {
    ...
    for (uint32_t i = 0u; i < sizeof(commands) / sizeof(commands[0]); i++) {
        boot_services_log("nexbox32: RUN net state command");
        kprint("nexbox32: RUN %s\n", commands[i]);
        if (!boot_user_services_run_command_arch(commands[i], &process) ||
            process.state != PROCESS_STATE_EXITED ||
            process.exit_code != 0) {
            kprint("nexbox32: FAIL %s status=%d\n",
                   commands[i],
    ...
    boot_services_log("rtl8139: command smoke OK");
    return 1;
}

static int smoke_services_run_hda(void) {
    ...
    kprint("hda: backend smoke OK bdf=%u:%u.%u codec=%x gcap=%x\n",
           ...
           status.codec_mask,
           status.gcap);
    boot_services_log("nexbox32: RUN /cmd/nexbox hda");
    if (!boot_user_services_run_command_arch("/cmd/nexbox hda", &process) ||
        process.state != PROCESS_STATE_EXITED ||
        process.exit_code != 0) {
    ...
    boot_services_log("nexbox32: PASS /cmd/nexbox hda");
    return 1;
}

static int smoke_services_run_ac97(void) {
    ...
    kprint("ac97: backend smoke OK bdf=%u:%u.%u codec=%x io=%x:%x\n",
           ...
           status.nambar,
           status.nabmbar);
    boot_services_log("nexbox32: RUN /cmd/nexbox ac97");
    if (!boot_user_services_run_command_arch("/cmd/nexbox ac97", &process) ||
        process.state != PROCESS_STATE_EXITED ||
        process.exit_code != 0) {
    ...
    boot_services_log("nexbox32: PASS /cmd/nexbox ac97");
    boot_services_log("nexbox32: RUN /cmd/nexbox wav --smoke /system/audio-smoke.wav");
    if (!boot_user_services_run_command_arch("/cmd/nexbox wav --smoke /system/audio-smoke.wav", &process) ||
        process.state != PROCESS_STATE_EXITED ||
        process.exit_code != 0) {
    ...
    boot_services_log("nexbox32: PASS /cmd/nexbox wav --smoke /system/audio-smoke.wav");
    return 1;
}

static int smoke_services_run_gfx_editor(void) {
    static const char *commands[] = {
        "/cmd/nexbox fb",
        "/cmd/nexbox fb --smoke",
        "/cmd/nexbox fb --blit-smoke",
    ...
    boot_user_services_init_config(&config);
    if (config.ops == 0 || config.ops->run_command == 0) {
        boot_services_log("gfx/editor smoke unavailable");
        return 0;
    }
    for (uint32_t i = 0u; i < sizeof(commands) / sizeof(commands[0]); i++) {
    ...
    boot_services_log("gfx/editor: smoke PASS");
    return 1;
}
...
int smoke_services_run_backend(int ahci,
                           ...
                           int ac97,
                           int gfx_editor) {
    if (ahci && !smoke_services_run_ahci_rw()) {
        return 0;
    }
    if (usb && !smoke_services_run_usb_rw()) {
        return 0;
    }
    if (usb_hid && !smoke_services_run_usb_hid()) {
        return 0;
    }
    if (rtl8139 && !smoke_services_run_rtl8139()) {
        return 0;
    }
    if (hda && !smoke_services_run_hda()) {
        return 0;
    }
    if (ac97 && !smoke_services_run_ac97()) {
        return 0;
    }
    if (gfx_editor && !smoke_services_run_gfx_editor()) {
        return 0;
    }
    return 1;
}
