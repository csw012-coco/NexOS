...
#define driver_alloc_pages driver_elf32_legacy_alloc_pages
#define driver_alloc_pages_below driver_elf32_legacy_alloc_pages_below
#define driver_free_pages driver_elf32_legacy_free_pages
#define driver_mmio_map driver_elf32_legacy_mmio_map
#define driver_str_len driver_elf32_legacy_str_len
#define driver_streq driver_elf32_legacy_streq
#define driver_starts_with driver_elf32_legacy_starts_with
#define driver_memcpy driver_elf32_legacy_memcpy
#define driver_memmove driver_elf32_legacy_memmove
#define driver_memset driver_elf32_legacy_memset
#define driver_audio_register_device driver_elf32_legacy_audio_register_device
#define driver_ac97_publish_device driver_elf32_legacy_ac97_publish_device
#define driver_hda_publish_device driver_elf32_legacy_hda_publish_device
#define driver_pci_find_by_class driver_elf32_legacy_pci_find_by_class
#define driver_pci_find_by_id driver_elf32_legacy_pci_find_by_id
#define driver_pci_read8 driver_elf32_legacy_pci_read8
#define driver_pci_read16 driver_elf32_legacy_pci_read16
#define driver_pci_read32 driver_elf32_legacy_pci_read32
#define driver_pci_write8 driver_elf32_legacy_pci_write8
#define driver_pci_write16 driver_elf32_legacy_pci_write16
#define driver_pci_write32 driver_elf32_legacy_pci_write32
#define driver_io_in8 driver_elf32_legacy_io_in8
#define driver_io_in16 driver_elf32_legacy_io_in16
#define driver_io_in32 driver_elf32_legacy_io_in32
#define driver_io_out8 driver_elf32_legacy_io_out8
#define driver_io_out16 driver_elf32_legacy_io_out16
#define driver_io_out32 driver_elf32_legacy_io_out32
#define driver_timer_current_ticks driver_elf32_legacy_timer_current_ticks
#define driver_timer_hz driver_elf32_legacy_timer_hz
#define driver_cpu_wait_for_event driver_elf32_legacy_cpu_wait_for_event
#define driver_cpu_relax driver_elf32_legacy_cpu_relax

static struct driver_elf32_module_allocation g_driver_elf32_module_allocs[DRIVER_ELF32_MODULE_ALLOC_MAX_COUNT];
static uint32_t g_driver_elf32_next_virt = DRIVER_ELF32_MODULE_VIRT_BASE;

uint64_t __udivdi3(uint64_t num, uint64_t den);
uint64_t __umoddi3(uint64_t num, uint64_t den);
int hda_i386_publish_status(const struct hda_status *status);

static struct driver_elf32_module_allocation *driver_elf32_alloc_slot(void) {
    for (uint32_t i = 0u; i < DRIVER_ELF32_MODULE_ALLOC_MAX_COUNT; i++) {
        if (g_driver_elf32_module_allocs[i].virt == 0) {
            return &g_driver_elf32_module_allocs[i];
        }
    }
    return 0;
}

static struct driver_elf32_module_allocation *driver_elf32_alloc_find(void *virt,
                                                                    uint32_t page_count) {
    if (virt == 0 || page_count == 0u) {
        return 0;
    }
    for (uint32_t i = 0u; i < DRIVER_ELF32_MODULE_ALLOC_MAX_COUNT; i++) {
        if (g_driver_elf32_module_allocs[i].virt == virt &&
            g_driver_elf32_module_allocs[i].page_count == page_count) {
            return &g_driver_elf32_module_allocs[i];
        }
    }
    return 0;
...
void *driver_elf32_alloc_pages(uint32_t page_count,
    ...
    struct driver_elf32_module_allocation *record;
    uint32_t virt;
    ...
    if (page_count == 0u || page_count > DRIVER_ELF32_MODULE_ALLOC_MAX_PAGES) {
        return 0;
    }
    record = driver_elf32_alloc_slot();
    if (record == 0) {
        return 0;
    }
    for (uint32_t i = 0u; i < DRIVER_ELF32_MODULE_ALLOC_MAX_PAGES; i++) {
        record->phys_pages[i] = I386_PMM_INVALID_PAGE;
    }
    virt = g_driver_elf32_next_virt;
    if (virt + page_count * DRIVER_ELF32_PAGE_SIZE < virt) {
        return 0;
    }
    for (uint32_t i = 0u; i < page_count; i++) {
        ...
        if (phys == I386_PMM_INVALID_PAGE ||
            !i386_paging_map_page(virt + i * DRIVER_ELF32_PAGE_SIZE,
                                  phys,
                                  1,
                                  0)) {
            if (phys != I386_PMM_INVALID_PAGE) {
            ...
            for (uint32_t u = 0u; u < i; u++) {
                ...
                (void)i386_paging_unmap_page(virt + u * DRIVER_ELF32_PAGE_SIZE,
                                             &ignored);
            }
    ...
    memset((void *)(uintptr_t)virt, 0, page_count * DRIVER_ELF32_PAGE_SIZE);
    record->virt = (void *)(uintptr_t)virt;
    record->page_count = page_count;
    g_driver_elf32_next_virt += page_count * DRIVER_ELF32_PAGE_SIZE;
    if (phys_out != 0) {
        *phys_out = record->phys;
    }
    if (alloc_size_out != 0) {
        *alloc_size_out = page_count * DRIVER_ELF32_PAGE_SIZE;
    }
    return record->virt;
}
...
void driver_elf32_free_pages(void *virt, uint32_t page_count) {
    ...
    record = driver_elf32_alloc_find(virt, page_count);
    if (record == 0) {
        return;
    }
...
void *driver_alloc_pages(uint32_t page_count, uint64_t *phys_out) {
    ...
    void *virt = driver_elf32_alloc_pages(page_count, &phys, 0);
...
void *driver_alloc_pages_below(uint32_t page_count,
    ...
    if (virt != 0 && phys_out != 0 && *phys_out >= max_phys_exclusive) {
        driver_elf32_free_pages(virt, page_count);
        *phys_out = 0u;
        return 0;
    }
    return virt;
...
static void driver_elf32_pci_copy(struct driver_pci_device *out,
...
static void driver_elf32_module_log(const char *fmt, ...) {
    va_list ap;
...
static const struct driver_elf32_kernel_symbol g_driver_elf32_kernel_symbols[] = {
    { "driver_log", (uint32_t)(uintptr_t)driver_elf32_module_log },
...
int driver_elf32_kernel_symbol_resolve(const char *name, uint32_t *value_out) {
    ...
    for (uint32_t i = 0u; g_driver_elf32_kernel_symbols[i].name != 0; i++) {
        if (streq(g_driver_elf32_kernel_symbols[i].name, name)) {
            *value_out = g_driver_elf32_kernel_symbols[i].value;
            return 1;
        }
    }
    return 0;
