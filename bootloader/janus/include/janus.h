...
#define KERNEL_LOAD_ADDR 0x01000000u
#define MODULE_LOAD_ADDR 0x02000000u
#define BOOT_INFO_ADDR 0x00018000u
#define JANUS_PML4_ADDR 0x00021000u
#define JANUS_PDPT_ADDR 0x00022000u
#define JANUS_PD_ADDR   0x00023000u
#define JANUS_PT_POOL_ADDR     0x00027000u
#define JANUS_PT_POOL_END_ADDR 0x00040000u
#define JANUS_HIGHER_HALF_LOAD_ADDR 0x00800000u

#define KERNEL_SCRATCH_MAX (4 * 1024 * 1024u)
#define JANUS_CMDLINE_MAX 160u
...
struct janus_bios_e820_entry {
    uint64_t base;
    uint64_t length;
    uint32_t type;
} __attribute__((packed));

struct janus_vbe_controller_info {
    char signature[4];
    uint16_t version;
...
struct janus_services {
    ...
    int (*bios_init_framebuffer)(struct janus_console_info *info, uint16_t width, uint16_t height, uint8_t bpp);
    int (*bios_set_text_mode)(void);
};

struct janus_stage3_params {
    struct janus_boot_info boot_info;
    uint32_t memmap_count;
    struct janus_memmap_entry memmap[JANUS_MAX_MEMMAP];
    const struct janus_services *services;
};
...
uint16_t bios_read_key(void);
uint16_t bios_poll_key(void);
int bios_query_e820(struct janus_memmap_entry *entries, uint32_t *entry_count, uint32_t max_entries);
int bios_init_framebuffer(struct janus_console_info *info, uint16_t width, uint16_t height, uint8_t bpp);
int bios_set_text_mode(void);
void stage3_set_services(const struct janus_services *services);
void janus_enter_long_mode(uint32_t pml4_phys, uint32_t entry_lo, uint32_t entry_hi, uint32_t boot_info_ptr);
