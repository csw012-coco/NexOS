...
static struct janus_vbe_mode_info *const bios_vbe_mode_info =
    (struct janus_vbe_mode_info *)(uintptr_t)BIOS_RM_VBE_MODE_INFO_ADDR;
static struct janus_vbe_controller_info *const bios_vbe_controller_info =
    (struct janus_vbe_controller_info *)(uintptr_t)BIOS_RM_VBE_CTRL_INFO_ADDR;
...
uint16_t bios_poll_key(void) {
    ...
    if (bios_key_extended && code == 0x48) {
        bios_key_extended = 0;
        return 0x4800;
    }
    if (bios_key_extended && code == 0x50) {
        bios_key_extended = 0;
        return 0x5000;
    }
    bios_key_extended = 0;
...
int bios_init_framebuffer(struct janus_console_info *info, uint16_t width, uint16_t height, uint8_t bpp) {
    ...
    if (selected_mode != 0 && vbe_query_mode(selected_mode) == 0) {
        ...
        info->type = JANUS_CONSOLE_FRAMEBUFFER;
        info->flags = 1;
...
int bios_set_text_mode(void) {
    ...
    r.eflags = 0x00000202u;
    if (bios_interrupt(0x10, &r, &r) != 0) {
        debug_puts("bios: set text mode failed\n");
        return -1;
    }
    return 0;
}
