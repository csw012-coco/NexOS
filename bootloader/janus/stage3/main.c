...
#define JANUS_PATH_MAX 64

struct menu_entry {
    char label[32];
    char kernel[JANUS_PATH_MAX];
    char cmdline[JANUS_CMDLINE_MAX];
    char modules[JANUS_MAX_MODULES][JANUS_PATH_MAX];
...
static struct janus_boot_info *const proto = (struct janus_boot_info *)BOOT_INFO_ADDR;
static struct janus_memmap_entry *const proto_memmap =
    (struct janus_memmap_entry *)(BOOT_INFO_ADDR + sizeof(struct janus_boot_info));
static struct janus_module *const proto_modules =
    (struct janus_module *)(BOOT_INFO_ADDR + sizeof(struct janus_boot_info) + sizeof(struct janus_memmap_entry) * JANUS_MAX_MEMMAP);
static char *const proto_cmdline =
    (char *)(BOOT_INFO_ADDR + sizeof(struct janus_boot_info) + sizeof(struct janus_memmap_entry) * JANUS_MAX_MEMMAP
        + sizeof(struct janus_module) * JANUS_MAX_MODULES);
static uint32_t page_table_next_free;
static uint32_t g_boot_payload_end = JANUS_HIGHER_HALF_LOAD_ADDR;

enum bootloader_video_mode {
    BOOTLOADER_VIDEO_TEXT = 0,
    BOOTLOADER_VIDEO_FB = 1
};
...
static enum bootloader_video_mode g_video_mode = BOOTLOADER_VIDEO_TEXT;
static int g_bootloader_fb_active;
static struct janus_console_info g_bootloader_fb_console;
...
static void set_default_menu(void) {
    g_video_mode = BOOTLOADER_VIDEO_TEXT;
    menu_count = 1;
...
static void parse_global_video_mode(const char *value) {
    if (strcmp(value, "fb") == 0 || strcmp(value, "framebuffer") == 0) {
        g_video_mode = BOOTLOADER_VIDEO_FB;
    } else if (strcmp(value, "text") == 0 || strcmp(value, "vga") == 0) {
        g_video_mode = BOOTLOADER_VIDEO_TEXT;
    }
}
...
static void parse_config_command(char *line, int *in_menuentry) {
    ...
    if (strncmp(command, "menuentry", 9) == 0 && command[9] == '\0') {
        ...
        *in_menuentry = 1;
        ...
        if (entry->module_count < JANUS_MAX_MODULES && read_token(&cur, value, sizeof(value))) {
...
static void parse_config(char *text) {
    ...
    while (*line != '\0' && menu_count < MAX_MENU_ENTRIES) {
        ...
        if (*cur == '\0') {
        ...
        } else if (!in_menuentry && strncmp(cur, "video_mode=", 11) == 0) {
            parse_global_video_mode(cur + 11);
            ...
            if (menu_entries[menu_count].module_count < JANUS_MAX_MODULES) {
...
static void load_config_or_default(void) {
    ...
    if (fat16_open_path(&fat, "BOOT/JANUS.CFG", &cfg) != 0 &&
        fat16_open_path(&fat, "JANUS.CFG", &cfg) != 0) {
        set_default_menu();
        return;
    }
    if (cfg.size >= 64 * 1024u) {
        fail("config too large");
    }
...
static void build_memory_map(const struct janus_stage3_params *params) {
    ...
    if (params->memmap_count != 0) {
        ...
        memcpy(proto_memmap, params->memmap, sizeof(struct janus_memmap_entry) * params->memmap_count);
        return;
    }
...
static void warn_framebuffer_text_fallback(void) {
    console_set_color(0x1E);
    console_clear();
    console_puts("Janus warning\n\n");
    console_set_color(0x0F);
    console_puts("The requested VBE framebuffer mode is not available.\n");
    console_puts("Booting will continue in VGA text mode.\n\n");
...
static void restore_bootloader_text_mode(void) {
    if (!g_bootloader_fb_active) {
        return;
    }

    if (bios_set_text_mode() != 0) {
        debug_puts("stage3: text mode restore failed\n");
    }
    console_configure_text();
    console_clear();
    g_bootloader_fb_active = 0;
}

static void configure_bootloader_video_mode(void) {
    struct video_request request;

    if (g_video_mode != BOOTLOADER_VIDEO_FB) {
        restore_bootloader_text_mode();
        return;
    }
    ...
    if (menu_count != 0 && parse_video_request(&menu_entries[0], &request) == 0) {
        if (request.width == 0) {
            request.width = 800;
        }
        if (request.height == 0) {
    ...
    if (bios_init_framebuffer(&g_bootloader_fb_console,
                              request.width,
                              request.height,
                              request.bpp) != 0) {
        debug_puts("stage3: bootloader framebuffer request failed\n");
        g_video_mode = BOOTLOADER_VIDEO_TEXT;
        g_bootloader_fb_active = 0;
        return;
    }

    console_configure_framebuffer(&g_bootloader_fb_console);
    console_clear();
    g_bootloader_fb_active = 1;
}

static void build_console_info(const struct janus_console_info *selected_console) {
    memset(&proto->console, 0, sizeof(proto->console));
    if (selected_console != 0 && selected_console->type != JANUS_CONSOLE_NONE) {
        memcpy(&proto->console, selected_console, sizeof(proto->console));
        return;
    }

    proto->console.type = JANUS_CONSOLE_TEXT;
...
static void build_identity_page_tables_4g(void) {
    uint64_t *pml4 = (uint64_t *)(uintptr_t)JANUS_PML4_ADDR;
    uint64_t *pdpt = (uint64_t *)(uintptr_t)JANUS_PDPT_ADDR;
    uint64_t *pd = (uint64_t *)(uintptr_t)JANUS_PD_ADDR;

    memset(pml4, 0, 0x1000);
    memset(pdpt, 0, 0x1000);
    memset(pd, 0, 0x4000);
    page_table_next_free = JANUS_PT_POOL_ADDR;

    pml4[0] = (uint64_t)JANUS_PDPT_ADDR | 0x03;

    for (uint32_t table = 0; table < 4; table++) {
        uint64_t *cur_pd = pd + table * 512;
        pdpt[table] = (uint64_t)(JANUS_PD_ADDR + table * 0x1000) | 0x03;
...
static uint64_t *alloc_page_table(void) {
    ...
    if (page_table_next_free >= JANUS_PT_POOL_END_ADDR) {
        fail("page table pool exhausted");
    }

    table = (uint64_t *)(uintptr_t)page_table_next_free;
...
static void prepare_proto(const struct janus_stage3_params *params, const struct menu_entry *entry,
                         const struct janus_console_info *selected_console) {
    const struct janus_boot_info *base_info = &params->boot_info;
    trace("prepare proto");
    memset(proto, 0, sizeof(*proto));
    memset(proto_memmap, 0, sizeof(struct janus_memmap_entry) * JANUS_MAX_MEMMAP);
    memset(proto_modules, 0, sizeof(struct janus_module) * JANUS_MAX_MODULES);
    memset(proto_cmdline, 0, JANUS_CMDLINE_MAX);

    proto->hdr.magic = JANUS_MAGIC;
    proto->hdr.version = JANUS_PROTOCOL_VERSION;
    ...
    copy_string(proto_cmdline, entry->cmdline, JANUS_CMDLINE_MAX);
...
static void load_elf32_and_jump(const struct janus_stage3_params *params, const struct menu_entry *entry, uint8_t *kernel) {
    uint32_t lowest_addr = 0xFFFFFFFFu;
    uint32_t highest_addr = 0;
    struct janus_console_info selected_console;
    ...
    ((void (*)(const struct janus_boot_info *))ehdr->e_entry)(proto);
    fail("kernel returned");
}

static void load_elf64_and_jump(const struct janus_stage3_params *params, const struct menu_entry *entry, uint8_t *kernel) {
    ...
    next_phys = g_boot_payload_end > JANUS_HIGHER_HALF_LOAD_ADDR
                    ? g_boot_payload_end
                    : JANUS_HIGHER_HALF_LOAD_ADDR;
    ...
    janus_enter_long_mode(JANUS_PML4_ADDR, (uint32_t)ehdr->e_entry, (uint32_t)(ehdr->e_entry >> 32), BOOT_INFO_ADDR);
    fail("long mode returned");
}

static void load_kernel_and_jump(const struct janus_stage3_params *params, const struct menu_entry *entry) {
...
void stage3_main(const struct janus_boot_info *raw_params) {
    const struct janus_stage3_params *params = (const struct janus_stage3_params *)raw_params;
    const struct janus_boot_info *boot_info = &params->boot_info;
    ...
    trace("config ready");
    configure_bootloader_video_mode();
