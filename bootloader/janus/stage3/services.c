...
static const struct janus_services *g_services;
...
uint16_t bios_poll_key(void) {
    if (g_services == 0 || g_services->bios_poll_key == 0) {
        return 0;
    }
    return g_services->bios_poll_key();
}
...
int bios_set_text_mode(void) {
    if (g_services == 0 || g_services->bios_set_text_mode == 0) {
        return -1;
    }
    return g_services->bios_set_text_mode();
}
