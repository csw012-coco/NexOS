...
#define JANUS_UEFI_DEFAULT_BOOT_LBA 2048u
#define JANUS_UEFI_DEFAULT_BOOT_SECTORS 98304u
#define JANUS_UEFI_IDENTITY_LIMIT (512ull * 1024ull * 1024ull * 1024ull)
#define JANUS_UEFI_MAX_PATH 160u
#define JANUS_UEFI_MAX_MENU_ENTRIES 8u
#define JANUS_UEFI_LABEL_MAX 64u
#define JANUS_UEFI_MENU_POLL_USEC 10000u
#define JANUS_UEFI_MARQUEE_STEP_USEC 250000u
#define JANUS_UEFI_MARQUEE_PAUSE_FRAMES 8u
#define JANUS_UEFI_MARQUEE_MIN_OVERFLOW 4u
...
struct efi_simple_text_output_protocol {
    ...
    void *QueryMode;
    void *SetMode;
    EFI_STATUS (*SetAttribute)(struct efi_simple_text_output_protocol *self, UINTN attribute);
    EFI_STATUS (*ClearScreen)(struct efi_simple_text_output_protocol *self);
    EFI_STATUS (*SetCursorPosition)(struct efi_simple_text_output_protocol *self, UINTN column, UINTN row);
    void *EnableCursor;
    void *Mode;
};
...
struct efi_simple_text_input_protocol {
    EFI_STATUS (*Reset)(struct efi_simple_text_input_protocol *self, uint8_t extended_verification);
...
struct boot_config {
    char label[JANUS_UEFI_LABEL_MAX];
    char kernel[JANUS_UEFI_MAX_PATH];
    char cmdline[JANUS_CMDLINE_MAX];
    char modules[JANUS_MAX_MODULES][JANUS_UEFI_MAX_PATH];
    uint32_t module_count;
};

struct uefi_boot_block {
    struct janus_boot_info info;
    char cmdline[JANUS_CMDLINE_MAX];
    struct janus_memmap_entry memmap[JANUS_MAX_MEMMAP];
    struct janus_module modules[JANUS_MAX_MODULES];
};
...
static struct boot_config g_menu_entries[JANUS_UEFI_MAX_MENU_ENTRIES];
static uint32_t g_menu_count;
...
static EFI_GUID g_acpi_10_table_guid = {
    0xeb9d2d30u, 0x2d88u, 0x11d3u, {0x9a, 0x16, 0x00, 0x90, 0x27, 0x3f, 0xc1, 0x4d}
};

static EFI_GUID g_acpi_20_table_guid = {
...
static int guid_equal(const EFI_GUID *lhs, const EFI_GUID *rhs) {
    return lhs != 0 && rhs != 0 && memcmp(lhs, rhs, sizeof(*lhs)) == 0;
}
...
static void efi_clear_screen(void) {
    if (g_st != 0 && g_st->ConOut != 0 && g_st->ConOut->ClearScreen != 0) {
...
static void config_init_default(struct boot_config *config) {
    ...
    str_copy_limit(config->label, sizeof(config->label), "NexOS");
...
static void init_menu_entry(struct boot_config *config) {
    memset(config, 0, sizeof(*config));
}

static void finish_menu_entry(uint32_t *entry_index) {
    struct boot_config *entry = &g_menu_entries[*entry_index];

    if (entry->kernel[0] == '\0') {
        return;
    }
    if (entry->label[0] == '\0') {
        str_copy_limit(entry->label, sizeof(entry->label), "Unnamed");
    }
    g_menu_count++;
    if (g_menu_count < JANUS_UEFI_MAX_MENU_ENTRIES) {
        *entry_index = g_menu_count;
        init_menu_entry(&g_menu_entries[*entry_index]);
    }
}
...
static void parse_config_command(char *line, uint32_t *entry_index, int *in_menuentry) {
    ...
    if (*entry_index >= JANUS_UEFI_MAX_MENU_ENTRIES) {
        return;
    }
    ...
    if (strcmp(command, "menuentry") == 0) {
        ...
        *in_menuentry = 1;
        ...
        if (entry->module_count < JANUS_MAX_MODULES && read_config_token(&cur, value, sizeof(value))) {
...
static void parse_config(const char *text, uint32_t size) {
    ...
    while (cur < end && entry_index < JANUS_UEFI_MAX_MENU_ENTRIES) {
        ...
        if (key_equals(key_start, (uint32_t)(key_end - key_start), "LABEL")) {
            if (g_menu_entries[entry_index].kernel[0] != '\0') {
                finish_menu_entry(&entry_index);
                if (g_menu_count >= JANUS_UEFI_MAX_MENU_ENTRIES) {
                    break;
                }
            }
            copy_span(g_menu_entries[entry_index].label,
                      sizeof(g_menu_entries[entry_index].label),
                      value_start,
                      value_end);
        } else if (key_equals(key_start, (uint32_t)(key_end - key_start), "KERNEL")) {
            copy_span(g_menu_entries[entry_index].kernel,
                      sizeof(g_menu_entries[entry_index].kernel),
                      value_start,
                      value_end);
        } else if (key_equals(key_start, (uint32_t)(key_end - key_start), "CMDLINE")) {
            copy_span(g_menu_entries[entry_index].cmdline,
                      sizeof(g_menu_entries[entry_index].cmdline),
                      value_start,
                      value_end);
        } else if (key_equals(key_start, (uint32_t)(key_end - key_start), "MODULE") &&
                   g_menu_entries[entry_index].module_count < JANUS_MAX_MODULES) {
            uint32_t module_index = g_menu_entries[entry_index].module_count;

            copy_span(g_menu_entries[entry_index].modules[module_index],
                      sizeof(g_menu_entries[entry_index].modules[module_index]),
                      value_start,
                      value_end);
            g_menu_entries[entry_index].module_count++;
        }
    }
    if (g_menu_count < JANUS_UEFI_MAX_MENU_ENTRIES) {
        finish_menu_entry(&entry_index);
...
static int efi_poll_key(struct efi_input_key *key) {
    ...
    if (g_st == 0 || g_st->ConIn == 0 || g_st->ConIn->ReadKeyStroke == 0 || key == 0) {
        efi_fail("console input unavailable");
    }
    ...
    if (status != EFI_NOT_READY && efi_is_error(status)) {
        efi_fail("console input failed");
    }
    return 0;
}
    ...
    for (;;) {
        ...
        if (!efi_poll_key(&key)) {
            if (g_bs != 0 && g_bs->Stall != 0) {
                g_bs->Stall(JANUS_UEFI_MENU_POLL_USEC);
            }
            continue;
        }
        ch = key.UnicodeChar;
...
static struct efi_file_protocol *open_file(const char *path) {
    CHAR16 path16[JANUS_UEFI_MAX_PATH];
...
static void map_identity_range_4k(uint64_t base, uint64_t size) {
    ...
    if (end <= JANUS_UEFI_IDENTITY_LIMIT) {
        return;
    }
    if (start < JANUS_UEFI_IDENTITY_LIMIT) {
        start = JANUS_UEFI_IDENTITY_LIMIT;
    }
...
static void fill_gop_console(struct janus_console_info *console,
    ...
    if (gop == 0 || gop->Mode == 0 || gop->Mode->Info == 0 ||
        ...
        console->type = JANUS_CONSOLE_NONE;
        return;
    }
    info = gop->Mode->Info;
...
static void load_modules(const struct boot_config *config) {
    ...
    for (uint32_t i = 0; i < config->module_count && i < JANUS_MAX_MODULES; i++) {
        uint32_t size = 0;
        void *data = read_file_alloc(config->modules[i], &size);
        struct janus_module *module = &g_boot_block->modules[g_boot_block->info.module_count];

        if (data == 0) {
            efi_puts("janus64: module missing: ");
...
static uint32_t efi_mem_type_to_boot(uint32_t type, uint64_t base) {
    if (type == EfiConventionalMemory && base < JANUS_UEFI_IDENTITY_LIMIT) {
        return JANUS_MEMMAP_USABLE;
    }
    if (type == EfiACPIReclaimMemory) {
        return JANUS_MEMMAP_ACPI_RECLAIMABLE;
    }
    if (type == EfiACPIMemoryNVS) {
        return JANUS_MEMMAP_ACPI_NVS;
    }
    if (type == EfiUnusableMemory) {
        return JANUS_MEMMAP_BAD;
    }
    if (type == EfiLoaderCode || type == EfiLoaderData ||
        type == EfiBootServicesCode || type == EfiBootServicesData) {
        return JANUS_MEMMAP_BOOTLOADER_RECLAIMABLE;
    }
    return JANUS_MEMMAP_RESERVED;
}

static void append_memmap_entry(uint64_t base, uint64_t length, uint32_t type) {
    struct janus_memmap_entry *entries = g_boot_block->memmap;
    ...
    if (count != 0) {
        struct janus_memmap_entry *prev = &entries[count - 1u];
        if (prev->type == type && prev->base + prev->length == base) {
            prev->length += length;
            return;
        }
    }
    if (count >= JANUS_MAX_MEMMAP) {
        return;
    }
...
static void convert_memory_map(struct efi_memory_descriptor *map, UINTN map_size, UINTN desc_size) {
    ...
    for (UINTN i = 0; i < count; i++) {
        ...
        if (desc->Type == EfiConventionalMemory && base < JANUS_UEFI_IDENTITY_LIMIT &&
            end > JANUS_UEFI_IDENTITY_LIMIT) {
            append_memmap_entry(base, JANUS_UEFI_IDENTITY_LIMIT - base, JANUS_MEMMAP_USABLE);
            append_memmap_entry(JANUS_UEFI_IDENTITY_LIMIT,
                                end - JANUS_UEFI_IDENTITY_LIMIT,
...
static uint64_t find_acpi_rsdp_addr(void) {
    struct efi_configuration_table *tables;

    if (g_st == 0 || g_st->ConfigurationTable == 0) {
        return 0;
    }
    tables = (struct efi_configuration_table *)g_st->ConfigurationTable;
    for (UINTN i = 0; i < g_st->NumberOfTableEntries; i++) {
        if (guid_equal(&tables[i].VendorGuid, &g_acpi_20_table_guid)) {
            return (uint64_t)(uintptr_t)tables[i].VendorTable;
        }
    }
    for (UINTN i = 0; i < g_st->NumberOfTableEntries; i++) {
        if (guid_equal(&tables[i].VendorGuid, &g_acpi_10_table_guid)) {
            return (uint64_t)(uintptr_t)tables[i].VendorTable;
        }
    }
    return 0;
...
EFI_STATUS efi_main(EFI_HANDLE image_handle, struct efi_system_table *system_table) {
    ...
    g_menu_count = 1;
    cfg_data = read_file_alloc("BOOT/JANUS.CFG", &cfg_size);
    if (cfg_data == 0) {
        cfg_data = read_file_alloc("JANUS.CFG", &cfg_size);
    }
    if (cfg_data != 0) {
        parse_config((const char *)cfg_data, cfg_size);
        if (g_menu_count == 0) {
            config_init_default(&g_menu_entries[0]);
            g_menu_count = 1;
        }
    }
    selected = choose_menu_entry();
    config = &g_menu_entries[selected];
    efi_clear_screen();
    g_boot_block = (struct uefi_boot_block *)uefi_alloc_low_pages(sizeof(*g_boot_block));
    if (g_boot_block == 0) {
        efi_fail("boot info allocation failed");
    }
    g_boot_block->info.hdr.magic = JANUS_MAGIC;
    g_boot_block->info.hdr.version = JANUS_PROTOCOL_VERSION;
    g_boot_block->info.hdr.size = sizeof(g_boot_block->info);
    g_boot_block->info.boot_drive = 0x80u;
    g_boot_block->info.partition_lba = JANUS_UEFI_DEFAULT_BOOT_LBA;
    g_boot_block->info.partition_sectors = JANUS_UEFI_DEFAULT_BOOT_SECTORS;
    g_boot_block->info.acpi_rsdp_addr = find_acpi_rsdp_addr();
