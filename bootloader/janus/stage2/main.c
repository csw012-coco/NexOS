...
typedef void (*stage3_entry_t)(const struct janus_boot_info *boot_info);

extern uint16_t early_e820_count;
extern struct janus_bios_e820_entry early_e820_entries[];

static struct janus_stage3_params stage3_params;
static const struct janus_services stage3_services = {
    .bios_read_sectors = bios_read_sectors,
    .bios_read_key = bios_read_key,
    .bios_poll_key = bios_poll_key,
    .bios_init_framebuffer = bios_init_framebuffer,
    .bios_set_text_mode = bios_set_text_mode,
};
...
void stage2_main(uint32_t drive) {
    ...
    stage3_params.boot_info.hdr.magic = JANUS_MAGIC;
    stage3_params.boot_info.hdr.version = JANUS_PROTOCOL_VERSION;
    ...
    ((stage3_entry_t)STAGE3_LOAD_ADDR)((const struct janus_boot_info *)&stage3_params);
